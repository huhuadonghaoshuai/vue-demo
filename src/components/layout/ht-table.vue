<template>
  <div class="ht-table-cpn">
    <el-table class="table-demo" size="medium" :data="tbData ? tbData : tableData" stripe v-loading="isTableLoading" header-row-class-name="header-row"
      style="width:100%" @expand-change="handleTableHeader" :height="tbHeight ? tbHeight : ''">
      <slot></slot>    
    </el-table>
...
export default {
   ...
   */
  props:['ajaxConfig', 'paginationKey', 'listKey', 'tbData', 'tbHeight', 'cacheState'],
  data () {
    return {
      current: 1,
      size: 10,
    ...
    handleGetPaginationInfo() {
      ...
      if (paginationKey) {
        ajaxConfig.params = Object.assign({}, ajaxConfig.params, {
          ...
          [paginationKey.current]: this.$route.query.current || this.current
        })
      }
  ...
  mounted() {
    ...
    this.size = parseInt(this.$route.query.size) || 10
    if (this.ajaxConfig) {
      this.handleGetPaginationInfo()
    }
  }
